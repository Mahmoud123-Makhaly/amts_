...
const Page = ({ params, searchParams }: { searchParams: any; params: { locale: string; orderId: string } }) => {
  const t = useTranslate('ORDER_REJECTION');
      ...
      <Col className="mt-5">
        <div className="flex-col mt-5">
          <div className="flex-col gap-3 mt-5">
            <i className="fa-solid fa-circle-xmark close-icon text-danger font-60"></i>
          </div>

          <h4 className="mt-4 text-black fw-normal">
            {t.rich('MSG', {
              br: () => <br />,
              strong: chunks => <strong className="text-danger fw-normal">{chunks}</strong>,
              countdown,
            })}
          </h4>
        </div>
