...
const Page = async () => {
  ...
  else if (!promiseResponse[0].value.data?.data?.shipments?.at(0)?.deliveryAddress) {
    redirect('/checkout/add-address');
  } else if (!promiseResponse[0].value.data?.data?.availableShippingMethods?.at(0)?.code) {
    redirect('/checkout/add-payment');
  } else
    return (
