...
import { AppForm, ButtonMaker, FormFieldType, ImageMaker, Modal, ServiceCard } from '@components';
interface IGreaseCleaningProps {
  data: Array<{
    imgSrc: string;
    name: string;
    slug: string;
...
const GreaseCleaning = (props: IGreaseCleaningProps) => {
  const { data } = props;
  const t = useTranslate('COMP_Grease_Cleaning');
          ...
          <Row dir="ltr" className="flex-column-reverse flex-lg-row">
            <Col lg={5}>
              <div className="flex-col-start h-100 gap-3">
                <h4 className="fw-semibold">{t('GREASE_CLEANING_TITLE')}</h4>
                <p className="text-dark-gray">{t('GREASE_CLEANING_DESCRIPTION')}</p>
              </div>
            </Col>
            <Col lg={7}>
              <ImageMaker
